package employee.version1;

public class TestMain {
    public static void main(String[] args) {
        // TODO code application logic here
        MyDate date1 = new MyDate(13,8,2012);
        MyDate date2 = new MyDate(14,7,2003);


//        ----------------------------HOURLY EMPLOYEE----------------------------

        System.out.println("\n\n\nHOURLY EMPLOYEES:");

        HourlyEmployee Emp1 = new HourlyEmployee();
        HourlyEmployee Emp2 = new HourlyEmployee(17283049, "Franco Cortes", date1, date2, 50, 1000);
        HourlyEmployee Emp3 = new HourlyEmployee(22102293, "Marco Cortes", date1, date2);

        Emp1.setEmpID(20101102);
        Emp1.setEmpName("Eduardo Cortes");
        Emp1.setEmpDateHired(date1);
        Emp1.setEmpBirthDate(date2);
        Emp1.setTotalHoursWorked(40);
        Emp1.setRatePerHour(1000);

        System.out.println(Emp1);
        Emp1.computeSalary();
        Emp1.displayInfo();

//        --------------------------------------------------------------

        System.out.println(Emp2);
        Emp2.computeSalary();
        Emp2.displayInfo();

//        --------------------------------------------------------------

        System.out.println(Emp3);

        Emp3.setTotalHoursWorked(40);
        Emp3.setRatePerHour(1000);

        Emp3.computeSalary();
        Emp3.displayInfo();

//        ----------------------------PIECE WORKER EMPLOYEE----------------------------

        System.out.println("\n\n\nPIECE WORKER EMPLOYEES:");


        PieceWorkerEmployee EmpA = new PieceWorkerEmployee();
        PieceWorkerEmployee EmpB = new PieceWorkerEmployee(17283049, "Franco Cortes", date1, date2, 50, 1000);
        PieceWorkerEmployee EmpC = new PieceWorkerEmployee(22102293, "Marco Cortes", date1, date2);

        EmpA.setEmpID(20101102);
        EmpA.setEmpName("Eduardo Cortes");
        EmpA.setEmpDateHired(date1);
        EmpA.setEmpBirthDate(date2);
        EmpA.setTotalPiecesFinished(100);
        EmpA.setRatePerPiece(1000);

        System.out.println(EmpA);
        EmpA.computeSalary();
        EmpA.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpB);
        EmpB.computeSalary();
        EmpB.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpC);

        EmpC.setTotalPiecesFinished(50);
        EmpC.setRatePerPiece(1000);

        EmpC.computeSalary();
        EmpC.displayInfo();

        //        ----------------------------COMMISSION EMPLOYEE----------------------------

        System.out.println("\n\n\nCOMMISSION EMPLOYEES:");


        CommissionEmployee EmpA1 = new CommissionEmployee();
        CommissionEmployee EmpB1 = new CommissionEmployee(17283049, "Franco Cortes", date1, date2, 50000);
        CommissionEmployee EmpC1 = new CommissionEmployee(22102293, "Marco Cortes", date1, date2);

        EmpA1.setEmpID(20101102);
        EmpA1.setEmpName("Eduardo Cortes");
        EmpA1.setEmpDateHired(date1);
        EmpA1.setEmpBirthDate(date2);
        EmpA1.setTotalSales(20000);

        System.out.println(EmpA1);
        EmpA1.computeSalary();
        EmpA1.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpB1);
        EmpB1.computeSalary();
        EmpB1.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpC1);

        EmpC1.setTotalSales(500000);

        EmpC1.computeSalary();
        EmpC1.displayInfo();

        //        ----------------------------BASE PLUS COMMISSION EMPLOYEE----------------------------

        System.out.println("\n\n\nBASE PLUS COMMISSION EMPLOYEES:");

        BasePlusCommissionEmployee EmpA2 = new BasePlusCommissionEmployee();
        BasePlusCommissionEmployee EmpB2 = new BasePlusCommissionEmployee(17283049, "Franco Cortes", date1, date2, 50000, 25000);
        BasePlusCommissionEmployee EmpC2 = new BasePlusCommissionEmployee(22102293, "Marco Cortes", date1, date2);

        EmpA2.setEmpID(20101102);
        EmpA2.setEmpName("Eduardo Cortes");
        EmpA2.setEmpDateHired(date1);
        EmpA2.setEmpBirthDate(date2);
        EmpA2.setTotalSales(1000);
        EmpA2.setBaseSalary(250000);

        System.out.println(EmpA2);
        EmpA2.computeSalary();
        EmpA2.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpB2);
        EmpB2.computeSalary();
        EmpB2.displayInfo();

//        --------------------------------------------------------------

        System.out.println(EmpC2);

        EmpC2.setTotalSales(50);
        EmpC2.setBaseSalary(250000);

        EmpC2.computeSalary();
        EmpC2.displayInfo();


    }
}
